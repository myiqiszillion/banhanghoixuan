...
export const db = {
    ...
    async getTicketBalance(phone) {
        ...
        if (isPostgresConfigured) {
            const client = await pool.connect();
            try {
                // 1. Get tickets from PAID orders
                ...
                const orderTickets = parseInt(orderRes.rows[0]?.total_tickets || 0);

                // 2. Get bonus tickets from game_states
                const bonusRes = await client.query(
                    `SELECT COALESCE(bonus_tickets, 0) as bonus_tickets FROM game_states WHERE phone = $1`,
                    [phone]
                );
                const bonusTickets = parseInt(bonusRes.rows[0]?.bonus_tickets || 0);

                return orderTickets + bonusTickets;
            } catch (e) {
        ...
        if (isPostgresConfigured) {
            ...
            try {
                const { rows } = await client.query(
                    `SELECT used_tickets, COALESCE(bonus_tickets, 0) as bonus_tickets FROM game_states WHERE phone = $1`,
                    [phone]
                );
                if (rows[0]) {
                    return {
                        usedTickets: parseInt(rows[0].used_tickets || 0),
                        bonusTickets: parseInt(rows[0].bonus_tickets || 0)
                    };
                }
                return null;
            } catch (e) {
    ...
    async updateGameState(phone, state) {
        if (isPostgresConfigured) {
            ...
            try {
                await client.query(
                    `INSERT INTO game_states (phone, used_tickets, bonus_tickets, updated_at)
                     VALUES ($1, $2, $3, NOW())
                     ON CONFLICT (phone) DO UPDATE SET
                         used_tickets = $2,
                         bonus_tickets = COALESCE(game_states.bonus_tickets, 0) + $3 - COALESCE((SELECT bonus_tickets FROM game_states WHERE phone = $1), 0),
                         updated_at = NOW()`,
                    [phone, state.usedTickets, state.bonusTickets || 0]
                );
                return true;
            } catch (e) {
    ...
    async getAllGameStats() {
        if (isPostgresConfigured) {
            ...
            try {
                ...
                const gameStatesRes = await client.query(`SELECT phone, used_tickets, COALESCE(bonus_tickets, 0) as bonus_tickets FROM game_states`);
