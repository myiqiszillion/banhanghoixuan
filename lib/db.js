...
export const db = {
    ...
    async updateGameState(phone, state) {
        if (isPostgresConfigured) {
            ...
            try {
                await client.query(
                    `INSERT INTO game_states (phone, collected_cards, used_tickets, updated_at)
                     VALUES ($1, $2, $3, NOW())
                     ON CONFLICT (phone) DO UPDATE SET
                         collected_cards = $2,
                         used_tickets = $3,
                         updated_at = NOW()`,
                    [phone, JSON.stringify(state.collectedCards), state.usedTickets]
                );
                return true;
            } catch (e) {
    ...
    async getAllGameStats() {
        if (isPostgresConfigured) {
            ...
            try {
                ...
                // 2. Get all GAME STATES
                const gameStatesRes = await client.query(`SELECT phone, collected_cards, used_tickets FROM game_states`);
    ...
    async getPaidOrders() {
        if (isPostgresConfigured) {
            ...
            return global.mockStore.orders.filter(o => o.status === 'paid');
        }
    }
};
